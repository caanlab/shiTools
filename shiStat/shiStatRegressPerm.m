function [Beta, Pval, Stat, Perm] = shiStatRegressPerm(Y,X,n_iter)

% conducts permutation test of linear regression
%
% zhenhao shi
...
for i = 1:nVar
    ...
    for j = 1:n_iter
        resid_perm = resid_reduced(randperm(nObs));
    ...
    pval_beta(i+1) = sum(abs(Beta(i+1))<abs(beta_perm(:,i)))/n_iter;
    pval_tval(i+1) = sum(abs(tval(i+1))<abs(tval_perm(:,i)))/n_iter;
...
Perm.beta_perm = beta_perm;
Perm.tval_perm = tval_perm;
Perm.rho2_perm = rho2_perm;
Perm.fval_perm = fval_perm;
