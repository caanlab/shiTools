function [OutputImage,Vo,Yo,XYZo] = shiSpmImgCalc0(InputImages,Function,OutputImage,DO_WRITE_IMAGE,Interpolation,dt)

% runs spm_imcalc to calculate nifti image files voxel-wise
%
% shiSpmImgCalc0(InputImages,Function,OutputImage)
% shiSpmImgCalc0(InputImages,Function,OutputImage,DO_WRITE_IMAGE)
% shiSpmImgCalc0(InputImages,Function,OutputImage,DO_WRITE_IMAGE,Interpolation)  
% 
%   InputImages - a string or a cell array of strings for input image file
%                 name(s)
%   Function    - definition of calculation. for example '(i1+i2)./i3' to
%                 treat each input image individually, or 'mean(X)' to
%                 treat them as a vector.
%   OutputImage - a string for output image file name
%   DO_WRITE_IMAGE - may choose to not write image into file (default:true)
%   Interpolation - 0: nearest or 1: trilinear (default)
% 
%    ###########
% by Zhenhao Shi @ 2017-7-21
%    ###########
% 

if ~exist('dt','var') || isempty(dt)
    try
        DT = spm_vol(char(InputImages));
        dt = cell(length(DT),1);
        [dt{:}] = deal(DT.dt);
        dt=cell2mat(dt);
        dt = max(dt(:,1));
...
if ~exist('DO_WRITE_IMAGE','var') || isempty(DO_WRITE_IMAGE)
    DO_WRITE_IMAGE = true;
end
...
InputImages = char(InputImages);
if ~isempty(regexp(Function,'i[0-9]','once'))
    Mat = [];
elseif ~isempty(regexp(Function,'X','once'))
    Mat = 1;
else
    error('cannot find ''i?'' or ''X'' in expression');
end


PathOut = fileparts(OutputImage);
...
[Vo,Yo] = shi_spm_imcalc(DO_WRITE_IMAGE,InputImages,OutputImage,Function,{Mat,[],Interpolation,dt});
    [R,C,P]  = ndgrid(1:Vo.dim(1),1:Vo.dim(2),1:Vo.dim(3));
    RCP      = [R(:)';C(:)';P(:)'];
    RCP(4,:) = 1;
    XYZo      = Vo.mat(1:3,:)*RCP;
...
function [Vo,Y] = shi_spm_imcalc(DO_WRITE_IMAGE,Vi,Vo,f,flags,varargin)
