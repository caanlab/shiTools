function [Dvars_standardized,Dvars_raw,Dvars_spike,Stat] = shiSpmPreprocDvarsCalc(Img,Mask_inSkull,Img_mean)

% calculates DVARS following Afyouni & Nichols (2018), NeuroImage
%
...
if noMean
    if noMask
        Yimg = shiSpmRoiXtrVoxelwise(char(Img));
    else
        Mask_inSkull = char(Mask_inSkull);
        Yimg = shiSpmRoiXtrVoxelwise(char(Img),Mask_inSkull);
    end
    Yimg = Yimg{1}';
    Yimg_raw = Yimg;
else
    Img = cellstr(char(Img));
    Img_mean = cellstr(char(Img_mean));
    if noMask
        [~,Yall,~] = shiSpmResliceRead([Img;Img_mean]);
    else
        Mask_inSkull = char(Mask_inSkull);
        [~,~,~,Yall,~] = shiSpmMaskRead([Img;Img_mean],Mask_inSkull);
    end
    Yimg = bsxfun(@plus,Yall(:,:,:,1:end-1),(-mean(Yall(:,:,:,1:end-1),4)+Yall(:,:,:,end))); % remove mean of raw images, add mean image
    Yimg = reshape(Yimg,[numel(Yall(:,:,:,1)),length(Img)]);
    Yimg_raw = reshape(Yall(:,:,:,1:end-1),[numel(Yall(:,:,:,1)),length(Img)]);
end

mskOut = mean(Yimg_raw,2,'omitnan')<=0.1 & std(Yimg_raw,[],2,'omitnan')<=0.1;
Yimg = Yimg(~mskOut,:);

[Dvars_raw,Stat] = DVARSCalc(Yimg,'TestMethod','X2','Norm',100,'verbose',false);
Dvars_raw = [0 Dvars_raw]';
Dvars_standardized = [0 Stat.SDVARS_X2]';
Dvars_spike = [false Stat.pvals<0.05./(length(Dvars_raw)-1) & Stat.DeltapDvar>5]';
...
fnnf=mfilename; if ~nargin; help(fnnf); return; end 
clear fnnf;
%_________________________________________________________________________
...
% 
% if size(Mask,1)>1
%     error('only one mask image allowed')
% end
% 
% Img_skullstripped = cellstr(char(Img_skullstripped));
% 
% Y = shiSpmRoiXtrVoxelwise(Img_skullstripped,Mask);
% Y = Y{1};
% Med = median(Y(Y>0));
% 
% Y = Y./Med*100;
% dY_sq = diff(Y).^2;
% 
% nanvox = any(isnan(dY_sq));
% dY_sq = dY_sq(:,~nanvox);
% 
% Dvars = sqrt(mean(dY_sq,2)); % Power 2012, traditional
% Dvars_standardized = Dvars.^2/4; % 
