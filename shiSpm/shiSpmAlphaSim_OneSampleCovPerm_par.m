function [MaxClusterSize,MaxT] = shiSpmAlphaSim_OneSampleCovPerm_par(Img, NuisanceVar, WorkingDir, MaskImg, OutTxt, ConnectDef, Voxel_P, n_Iteration)
...
%     axis off
    ...
    shiSpmAlphaSim_OneSampleCovPerm_OneSampleRegress(PermDir{iter},Img,Cov(:,iter),NUI);
...
function shiSpmAlphaSim_OneSampleCovPerm_OneSampleRegress(PermDir,Img,Cov,Nui)
%%
matlabbatch{1}.spm.stats.factorial_design.dir = {char(PermDir)};
matlabbatch{1}.spm.stats.factorial_design.des.mreg.scans = Img;

matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(1).c = Cov;
matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(1).cname = 'OneSampleReg';
matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(1).iCC = 5;
% matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(1).iCFI = 1;
for j = 1:size(Nui,2)
    matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(j+1).c = Nui(:,j);
    matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(j+1).cname = sprintf('Nui #%d',j);
    matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(j+1).iCC = 5;
%     matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(j+1).iCFI = 1;
end
